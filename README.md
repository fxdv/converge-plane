...
Circle gives small and medium engineering teams the common issue loop done well:
...
```sh
git clone <this-repo> circle && cd circle
cp .env.example .env
...
cd server && go run ./cmd/circle       # API on :3001
...
| `web/` | Web application (Next.js) — forked from Tegon, being adapted for Circle |
...
The Circle web frontend is derived from
[Tegon](https://github.com/tegonhq/tegon) (v0.3.11-alpha), an AGPL-3.0
open-source issue tracker. Tegon's UI is forked and adapted for Circle; the
Go server is an original implementation of the same product contracts.
See [NOTICE](NOTICE) for the full attribution.

## License

Circle is licensed under the [GNU Affero General Public License v3.0 or
